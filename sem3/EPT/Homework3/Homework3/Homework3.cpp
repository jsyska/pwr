...
int main()
{
	cout << "-- Static initialization (without and params):" << endl;
		CTable tab_stat_def;
		CTable tab_stat_cus = CTable("test_stat", 25);
		cout << "\n-- Dynamic initialization (without and params):" << endl;
		CTable *p_tab_dyn_def = new CTable();
		CTable *p_tab_dyn_cus = new CTable("test_dyn", 20);
		cout << "\n-- Dynamic array initialization (without):" << endl;
	...
	for (int i = 0; i < p_tabs[0].iGetSize(); i++)
	{
